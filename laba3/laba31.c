...
{
  const struct book *lhs1=(const struct book*) lhs;
  const struct book *rhs2=(const struct book*) rhs;
  int t;
  t = strcmp(lhs1->name, rhs2->name);
  if (t != 0)
    return t;
}
...
{
  /* сортировка */
  qsort(catalog, MAXRECORDS, sizeof(struct book), compare);
  register int i;
